"""Explore smartwatch-related categories across multiple Amazon metadata files.

This script searches for smartwatches across Electronics, Health & Personal Care,
Sports & Outdoors, and Cell Phones & Accessories to identify the correct
categories and filtering logic.

Run this before creating the smartwatch preprocessing script.
"""
...
DATA_FILES = {
    ...
    "Health_and_Personal_Care": "hf://datasets/McAuley-Lab/Amazon-Reviews-2023/raw/meta_categories/meta_Health_and_Personal_Care.jsonl",
    "Sports_and_Outdoors": "hf://datasets/McAuley-Lab/Amazon-Reviews-2023/raw/meta_categories/meta_Sports_and_Outdoors.jsonl",
    "Cell_Phones_and_Accessories": "hf://datasets/McAuley-Lab/Amazon-Reviews-2023/raw/meta_categories/meta_Cell_Phones_and_Accessories.jsonl",
}

# Keywords to identify smartwatches
SMARTWATCH_KEYWORDS = [
    "smart watch",
    "smartwatch",
    "fitness watch",
...
# Keywords to exclude (accessories, not actual watches)
EXCLUDE_KEYWORDS = [
    "band",
    "strap",
    "charger",
...
def is_smartwatch(title: str) -> bool:
    """Check if title indicates a smartwatch (not accessory)."""
...
def explore_metadata_file(
    ...
    """Explore one metadata file for smartwatches."""
    print(f"\n{'='*80}")
    ...
    with tqdm(total=max_items, unit="items", desc=file_key) as pbar:
        for i, item in enumerate(open_jsonl_source(file_path)):
            ...
            # Check if this is a smartwatch
            if is_smartwatch(title):
                total_smartwatches += 1

                # Collect sample
                if len(smartwatch_samples) < max_samples:
                    smartwatch_samples.append(
                        {
                            "title": title,
    ...
    print(f"{'='*80}")
    print(f"Total smartwatches found: {total_smartwatches:,}")
    ...
    if total_smartwatches > 0:
        ...
        print(
            f"\nSample Smartwatch Items (showing {min(10, len(smartwatch_samples))}):"
        )
        for i, sample in enumerate(smartwatch_samples[:10], 1):
            print(f"\n{i}. {sample['title'][:80]}...")
    ...
    return {
        "file": file_key,
        "total_smartwatches": total_smartwatches,
        "main_categories": main_categories,
        "category_paths": category_paths,
        "samples": smartwatch_samples,
    }


def main():
    print("=" * 80)
    print("SMARTWATCH CATEGORY EXPLORATION")
    print("=" * 80)
    print("\nSearching for smartwatches across multiple metadata files...")
    ...
    total_across_all = sum(r["total_smartwatches"] for r in results.values())
    print(f"\nTotal smartwatches found: {total_across_all:,}")

    print("\nBreakdown by source file:")
    for file_key in DATA_FILES.keys():
        if file_key in results:
            count = results[file_key]["total_smartwatches"]
            pct = (
    ...
    print(
        """
Based on this exploration, consider the following for smartwatch preprocessing:

1. Which source file(s) have the most smartwatches?
...
- Create smartwatch_preprocess.py with appropriate filters
    """
    )
